...
from kaggle_environments.envs.orbit_wars.orbit_wars import Fleet, Planet  # type: ignore[reportMissingImports]
...
def planet_motion_types(planets, initial_by_id, comet_ids):
    motion_types = {}
    for planet in planets:
        ...
        if orbital_radius + initial[4] < ROTATION_RADIUS_LIMIT - 1e-9:
...
def planet_position_after_moves(planet, moves_done, initial_by_id, angular_velocity, current_step, comet_ids):
...
def circle_collision_time(start_x, start_y, dir_x, dir_y, speed, center_x, center_y, radius, max_time):
...
def board_exit_time(start_x, start_y, dir_x, dir_y, speed):
    exit_times = []
    velocity_x = dir_x * speed
    velocity_y = dir_y * speed
...
def segment_orbit_clearance(start, end, orbit_radius):
...
def moving_blocker_margin(source, blocker, angle, ships, hit_time, initial_by_id, angular_velocity, current_step, comet_ids):
...
def moving_blocker_margin_derivative(source, blocker, angle, ships, hit_time, initial_by_id, angular_velocity, current_step, comet_ids):
    speed = fleet_speed(ships, MAX_SPEED)
    dir_x = math.cos(angle)
    dir_y = math.sin(angle)
    start_x = source.x + dir_x * (source.radius + 0.1)
...
def moving_blocker_collision_time(source, blocker, angle, ships, initial_by_id, angular_velocity, current_step, comet_ids, max_time):
    speed = fleet_speed(ships, MAX_SPEED)
    dir_x = math.cos(angle)
    dir_y = math.sin(angle)
    start_x = source.x + dir_x * (source.radius + 0.1)
    ...
    def margin_at(hit_time):
        fleet_x = start_x + dir_x * speed * hit_time
        fleet_y = start_y + dir_y * speed * hit_time
        blocker_x, blocker_y = blocker_position(hit_time)
        return math.hypot(fleet_x - blocker_x, fleet_y - blocker_y) - blocker.radius

    def derivative_at(hit_time):
        fleet_x = start_x + dir_x * speed * hit_time
        fleet_y = start_y + dir_y * speed * hit_time
        blocker_x, blocker_y = blocker_position(hit_time)
        dx = fleet_x - blocker_x
...
def target_collision_time(source, target, angle, ships, initial_by_id, angular_velocity, current_step, comet_ids, max_turn):
    ...
    orbital_radius = math.hypot(initial[2] - CENTER, initial[3] - CENTER)
    if orbital_radius + initial[4] < ROTATION_RADIUS_LIMIT:
        intercept = find_moving_intercept_newton(
            source,
            target,
            ships,
            initial_by_id,
...
def find_attack_blocker(
    ...
    if target_hit_time is None:
        target_hit_time = target_collision_time(
            source,
            target,
            angle,
            ships,
    ...
    for planet in planets:
        ...
        if motion_type == "orbiting":
            ...
            if initial is not None:
                ...
                if segment_orbit_clearance((start_x, start_y), path_end, orbital_radius) > planet.radius + 1e-6:
                    continue
            blocker_time = moving_blocker_collision_time(
                source,
                planet,
                angle,
                ships,
            ...
            blocker_time = circle_collision_time(
                start_x,
                start_y,
                dir_x,
                dir_y,
        ...
        if blocker_time is not None and (best is None or blocker_time < best[0]):
            best = (
                blocker_time,
                {"kind": "planet", "planet_id": planet.id, "turn": math.ceil(blocker_time)},
            )
...
def moving_intercept_margin(source, target, ships, hit_time, initial_by_id, angular_velocity, current_step, comet_ids):
    speed = fleet_speed(ships, MAX_SPEED)
...
def moving_intercept_margin_derivative(source, target, ships, hit_time, initial_by_id, angular_velocity, current_step, comet_ids):
    speed = fleet_speed(ships, MAX_SPEED)
    target_x, target_y = planet_position_after_moves(
        target,
        hit_time,
...
def find_moving_intercept_newton(source, target, ships, initial_by_id, angular_velocity, current_step, comet_ids, max_turn=90):
    speed = fleet_speed(ships, MAX_SPEED)
    ...
    for seed in seed_times:
        hit_time = seed
        for _ in range(12):
            margin = moving_intercept_margin(
                source,
                target,
                ships,
                hit_time,
            ...
            derivative = moving_intercept_margin_derivative(
                source,
                target,
                ships,
                hit_time,
        ...
        final_margin = moving_intercept_margin(
            source,
            target,
            ships,
            hit_time,
...
def find_valid_attack_angle(source, target, ships, planets, initial_by_id, motion_types, angular_velocity, current_step, comet_ids):
    """Return a launch angle for a chosen target, or None if the shot is unsafe/useful nowhere."""
...
# Backward-compatible public names; canonical implementations are the copied opening-agent helpers above.
point_to_segment_distance = _point_to_segment_distance
point_to_segment_progress = _point_to_segment_progress
cross_2d = _cross_2d
segment_intersection_progress = _segment_intersection_progress
segment_to_segment_distance = _segment_to_segment_distance
segment_collision_progress = _segment_collision_progress
fleet_future_hit = _fleet_future_hit
incoming_events_by_target = _incoming_events_by_target
resolve_fleet_group_against_planet = _resolve_fleet_group_against_planet
project_target_before_arrival = _project_target_before_arrival
minimum_ships_to_own_after_arrival = _minimum_ships_to_own_after_arrival
projected_target_fleet_floor = _projected_target_fleet_floor
simulate_target_with_planned_fleet = _simulate_target_with_planned_fleet
survival_aware_target_fleet_floor = _survival_aware_target_fleet_floor
simulate_source_after_launch = _simulate_source_after_launch
max_safe_source_ships_to_send = _max_safe_source_ships_to_send


# Additional opening-agent helpers copied from agents/opening_agent_improved.py.

def _base_fleet_floor(target):
    return int(target.ships) + 1


def _target_fleet_floor(target, player_id, available_step, current_step):
...
def _base_cheap_production_score(target):
    return (5.0 * float(target.production)) / max(1.0, float(_base_fleet_floor(target)))


def _wait_turns_to_leave_one(source, ships_to_send):
...
def _opening_horizon(step):
    return int(step) + OPENING_SCORE_TURNS


def _net_opening_value(target, ships_to_send, capture_step, horizon_step):
...
def _target_filter_reason(planet, player_id, comet_ids, future_sources_by_id, blocked_quadrant):
...
def _first_capture_candidates(planets, player_id, comet_ids, future_sources_by_id, blocked_quadrant):
    eliminated = []
    for planet in planets:
...
def _travel_turns(source, target, ships_to_send, planets, initial_by_id, motion_types, angular_velocity, launch_step, comet_ids):
...
def _future_sources_by_id(
    ...
    step,
    comet_ids,
    incoming_events_by_target=None,
):
    if not fleets:
        return {}
...
def _best_matrix_assignment(rows, source_order):
    if not rows:
        return []
    ...
    columns = target_ids + [None] * len(source_ids)
...
def _choose_opening(obs, include_debug=False):
    ...
    incoming_events = _incoming_events_by_target(
        fleets,
        all_target_planets,
        initial_by_id,
        angular_velocity,
    ...
    def source_at_launch_step(source, reference_step, launch_step):
    ...
    def cached_travel_turns(source, target, ships_to_send, launch_step):
    ...
    def cached_target_fleet_floor(target, arrival_step, horizon_step):
    ...
    def cached_source_safe_ships(source, launch_step, horizon_step):
...
def choose_opening_moves(obs):
    moves, _debug = _choose_opening(obs)
    return moves


def opening_debug(obs):
    _moves, debug = _choose_opening(obs, include_debug=True)
    return debug


def moving_newton_intercept_agent(obs):
    player = get(obs, "player", 0)
    step = get(obs, "step", 0)
    raw_planets = get(obs, "planets", [])
    raw_initial_planets = get(obs, "initial_planets", raw_planets)
    angular_velocity = float(get(obs, "angular_velocity", 0.0))
    comet_ids = set(get(obs, "comet_planet_ids", []))

    planets = [Planet(*planet) for planet in raw_planets]
    initial_by_id = {planet[0]: planet for planet in raw_initial_planets}
    motion_types = planet_motion_types(planets, initial_by_id, comet_ids)
    my_planets = [planet for planet in planets if planet.owner == player and planet.ships >= 40]
    targets = [
        planet
        for planet in planets
        if motion_types[planet.id] == "orbiting"
    ]

    moves = []
    random.shuffle(my_planets)
    for source in my_planets:
        send_ships = max(1, int(source.ships // 2))
        target_order = [target for target in targets if target.id != source.id]
