...
def _production_cohort(production: int) -> str:
...
def _planet_overhead(planet: Planet) -> float:
    return float(planet.ships) / max(1.0, float(planet.production))


def _planetary_outlier_ids(
    planets: list[Planet],
    player_id: int,
    comet_ids: set[int],
) -> set[int]:
    ...
    overheads = [_planet_overhead(planet) for planet in target_planets]
    non_low_overheads = [
        _planet_overhead(planet)
        for planet in target_planets
        if int(planet.production) > 1
    ]
    global_q25 = _quantile(overheads, 0.25)
    global_median = _median(overheads)
    global_q75 = _quantile(overheads, 0.75)
    ...
    if non_low_overheads:
        non_low_q25 = _quantile(non_low_overheads, 0.25)
    ...
    for planet in target_planets:
        cohort = _production_cohort(int(planet.production))
        overheads_by_cohort[cohort].append(_planet_overhead(planet))

    cohort_thresholds: dict[str, float] = {}
    for cohort, cohort_overheads in overheads_by_cohort.items():
        cohort_q25 = _quantile(cohort_overheads, 0.25)
        cohort_median = _median(cohort_overheads)
        cohort_q75 = _quantile(cohort_overheads, 0.75)
        cohort_thresholds[cohort] = max(
            cohort_q75,
            cohort_median + 0.5 * max(0.0, cohort_q75 - cohort_q25),
        )

    outlier_ids: set[int] = set()
    for planet in target_planets:
        production = int(planet.production)
        cohort = _production_cohort(production)
        overhead = _planet_overhead(planet)
        ...
        is_strategy_overhead_outlier = (
            ...
            and overhead >= cohort_thresholds[cohort]
        )
        if is_low_production_outlier or is_global_overhead_outlier or is_strategy_overhead_outlier:
            outlier_ids.add(int(planet.id))
    return outlier_ids


def _ignored_planet_unlock_stage(progress: float) -> str:
    if progress >= 0.80:
        return "all"
    if progress >= 0.70:
        return "prod3"
...
def _ignored_planet_is_unlocked(production: int, progress: float) -> bool:
    production = int(production)
    if progress >= 0.80:
        return True
    if progress >= 0.70:
...
def _expansion_progress(
    planets: list[Planet],
    player_id: int,
    enemy_quadrant: tuple[int, int] | None,
    comet_ids: set[int],
    ignored_ids: set[int],
    future_sources_by_id: dict[int, int],
) -> dict[str, Any]:
    # Phased ignore uses progress through the normal non-enemy expansion pool.
    # Committed captures count as controlled so the agent can unlock deferred
    # high-production targets as soon as the expansion plan is effectively done.
    candidate_planets = [
        planet
        for planet in planets
        if int(planet.id) not in comet_ids
        and int(planet.id) not in ignored_ids
        and int(planet.production) > 0
        and (enemy_quadrant is None or _quadrant(planet) != enemy_quadrant)
    ]
    owned_ids = [
        int(planet.id)
    ...
    committed_ids = [
        ...
        for planet in candidate_planets
        if int(planet.owner) != int(player_id)
        and int(planet.id) in future_sources_by_id
    ]
    total = len(candidate_planets)
    controlled = len(owned_ids) + len(committed_ids)
    progress = 1.0 if total == 0 else controlled / total
    return {
        ...
        "stage": _ignored_planet_unlock_stage(progress),
    }
...
def _target_filter_reason(
    ...
    if int(planet.id) in locked_ignored_ids:
        return "phased ignore"
...
def _owned_planet_threat_status(
    target: Planet,
    player_id: int,
    current_step: int,
    horizon_step: int,
    ...
    owner = int(target.owner)
    ships = int(target.ships)
    horizon_turns = max(1, int(horizon_step) - int(current_step))
    target_events = incoming_events_by_target.get(int(target.id), {})
    incoming_enemy = sum(
        int(fleet_ships)
        for ships_by_owner in target_events.values()
        for owner_id, fleet_ships in ships_by_owner.items()
        if int(owner_id) != int(player_id)
    )
    incoming_friendly = sum(
        int(ships_by_owner.get(int(player_id), 0))
        for ships_by_owner in target_events.values()
    )
    lost_turn: int | None = None
    current_turn = 0
    for turn in sorted(int(turn) for turn in target_events if 1 <= int(turn) <= horizon_turns):
        if owner != -1:
            ships += int(target.production) * (turn - current_turn)
        turn_events = {int(owner_id): int(count) for owner_id, count in target_events.get(turn, {}).items()}
        if turn_events:
            owner, ships = _resolve_fleet_group_against_planet(owner, ships, turn_events)
        if lost_turn is None and owner != int(player_id):
            lost_turn = turn
        current_turn = turn

    if owner != -1 and current_turn < horizon_turns:
        ships += int(target.production) * (horizon_turns - current_turn)

    return {
        "target_id": int(target.id),
        "incoming_enemy": int(incoming_enemy),
        "incoming_friendly": int(incoming_friendly),
        "lost_turn": lost_turn,
        "lost_step": None if lost_turn is None else int(current_step) + int(lost_turn),
        "projected_owner": int(owner),
        "projected_ships": int(ships),
        "needs_reinforcement": bool(
            int(target.owner) == int(player_id)
            and incoming_enemy > 0
            and lost_turn is not None
        ),
    }


def _minimum_reinforcement_ships_to_survive(
    target: Planet,
    player_id: int,
    arrival_turns: int,
    horizon_turns: int,
    ...
    if not high_survives:
        high_simulation["capture_ships_needed"] = int(high)
        high_simulation["survival_extra_ships"] = 0
        return int(high), high_simulation
    ...
    if not survived:
        low = high
    best_simulation["capture_ships_needed"] = int(low)
    best_simulation["survival_extra_ships"] = 0
...
def _choose_opening(obs: Any, include_debug: bool = False) -> tuple[list[list[Any]], dict[str, Any]]:
    ...
    enemy_quadrant = _opponent_quadrant(home_quadrant) if home_quadrant is not None else None
    blocked_quadrant = None
    if home_quadrant is not None and step < OPPONENT_QUADRANT_ATTACK_STEP:
        blocked_quadrant = enemy_quadrant
    ...
    ignored_target_ids = _planetary_outlier_ids(planets, player_id, comet_ids)
    expansion_progress = _expansion_progress(
        planets,
        player_id,
        enemy_quadrant,
        comet_ids,
    ...
    progress = float(expansion_progress["progress"])
    planet_by_id = {int(planet.id): planet for planet in planets}
    unlocked_ignored_ids = {
        planet_id
        for planet_id in ignored_target_ids
        if _ignored_planet_is_unlocked(
            int(planet_by_id[planet_id].production),
            progress,
        )
    }
    locked_ignored_ids = ignored_target_ids - unlocked_ignored_ids
    debug["phased_ignore"] = {
        "progress": expansion_progress,
        "ignored_target_ids": sorted(ignored_target_ids),
        "locked_target_ids": sorted(locked_ignored_ids),
        "unlocked_target_ids": sorted(unlocked_ignored_ids),
    }
    ...
    def evaluate_reinforcement_source(
        ...
        if lost_turn is None:
            return None
        lost_turn = int(lost_turn)
        if lost_turn <= 0:
            return None

        horizon_turns = max(OPENING_SCORE_TURNS, lost_turn, 1)
        horizon_step = int(step) + horizon_turns
        target_events = incoming_events_by_target.get(int(target.id), {})
        other_incoming = sum(
            int(ships)
            for turn, ships_by_owner in target_events.items()
        ...
        source_capacity_before_loss = int(source.ships) + max(1, int(source.production)) * lost_turn - 1
        target_demand_bound = int(target.ships) + int(target.production) * horizon_turns + other_incoming + 2
        max_planned_ships = max(0, min(1000, source_capacity_before_loss, target_demand_bound))
        ...
        while planned_ships <= max_planned_ships:
            ...
            arrival_turns = int(wait_turns) + int(travel_turns)
            if arrival_turns > lost_turn:
                planned_ships += 1
                continue
            ...
            source_available_at_launch = max(
                0,
                int(source.ships) + max(1, int(source.production)) * int(wait_turns) - 1,
            )
            if planned_ships > source_available_at_launch or planned_ships > source_max_safe_ships:
                planned_ships += 1
                continue
            ...
            return {
                ...
                "angle": float(angle),
                "kind": "reinforce",
                "target_lost_turn": int(lost_turn),
                "target_lost_step": int(step) + int(lost_turn),
                "incoming_enemy": int(target_status.get("incoming_enemy", 0)),
                "incoming_friendly": int(target_status.get("incoming_friendly", 0)),
                "projected_owner": int(target_status.get("projected_owner", target.owner)),
                "projected_ships": int(target_status.get("projected_ships", target.ships)),
                "capture_ships_needed": int(floor_details.get("capture_ships_needed", floor_ships)),
                ...
                "arrival_owner": int(floor_details.get("arrival_owner", target.owner)),
                "arrival_ships": int(floor_details.get("arrival_ships", target.ships)),
                "final_owner": int(floor_details.get("final_owner", target.owner)),
                "final_ships": int(floor_details.get("final_ships", target.ships)),
                ...
                "source_survives_known_incoming": bool(
                    source_projection_details.get("source_survives_known_incoming", True)
                ),
    ...
    def reinforcement_sort_key(row: dict[str, Any]) -> tuple[int, int, int, int, int, int, int]:
        return (
            -int(row["ships_needed"]),
            -int(row["arrival_turns"]),
            -int(row["wait_turns"]),
            -int(row["travel_turns"]),
            int(source_by_id[int(row["source_id"])].production),
            int(source_by_id[int(row["source_id"])].ships),
            -int(row["source_id"]),
        )

    moves: list[list[Any]] = []
    source_by_id = {int(source.id): source for source in sources}
    reserved_reinforcement_sources: set[int] = set()
    reinforcement_rows: list[dict[str, Any]] = []
    owned_threats: list[tuple[Planet, dict[str, Any]]] = []
    horizon_step = _opening_horizon(step)
    for target in my_planets:
        if int(target.id) in comet_ids:
            continue
        target_status = _owned_planet_threat_status(
            target,
            player_id,
            step,
            horizon_step,
    ...
    for target, target_status in owned_threats:
        ...
        defense_sources = [
            ...
            and int(source.id) not in reserved_reinforcement_sources
        ]
        evaluations = [
            evaluation
            for source in defense_sources
            if (evaluation := evaluate_reinforcement_source(source, target, target_status)) is not None
        ]
        if not evaluations:
            continue
        selected_reinforcement = max(evaluations, key=reinforcement_sort_key)
        reserved_reinforcement_sources.add(int(selected_reinforcement["source_id"]))
        reinforcement_rows.append(selected_reinforcement)

        if int(selected_reinforcement["wait_turns"]) == 0:
            source = source_by_id.get(int(selected_reinforcement["source_id"]))
            ships_to_send = int(selected_reinforcement["ships_needed"])
            if source is not None and int(source.ships) >= ships_to_send + 1:
                moves.append(
                    [
                        int(selected_reinforcement["source_id"]),
                        float(selected_reinforcement["angle"]),
                        ships_to_send,
                    ]
                )

        selected = {
            "source_id": int(selected_reinforcement["source_id"]),
            "target_id": int(selected_reinforcement["target_id"]),
            "ships": int(selected_reinforcement["ships_needed"]),
            "available_step": int(selected_reinforcement["arrival_step"]),
            "kind": "reinforce",
            "assignment_score": 0.0,
            "source_available_step": int(step) + int(selected_reinforcement["wait_turns"]),
            "wait_turns": int(selected_reinforcement["wait_turns"]),
            "travel_turns": int(selected_reinforcement["travel_turns"]),
            "target_lost_step": int(selected_reinforcement["target_lost_step"]),
        }
        if include_debug:
    ...
    capture_sources = [
        source
        for source in sources
        if int(source.id) not in reserved_reinforcement_sources
    ]
    if not candidates or not capture_sources:
        if include_debug:
    ...
    selected_rows = _best_matrix_assignment(
        matrix_rows,
        [int(source.id) for source in capture_sources],
    )
